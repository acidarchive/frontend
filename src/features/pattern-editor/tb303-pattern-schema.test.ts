...
import { Note, Time } from '@/api/generated/model';

import { TB303PatternSchema } from './tb303-pattern-schema';
...
describe('TB303PatternSchema', () => {
  it('filters out empty trailing steps and assigns step numbers', () => {
    const pattern = {
      name: 'Test',
      steps: [
        { note: Note.C, time: Time.note },
        { note: Note.D, time: Time.note },
  ...
  it('removes undefined values', () => {
    const pattern = {
      name: 'Test',
      steps: [{ number: 1, note: Note.C, time: Time.note, accent: undefined }],
    };
  ...
  it('invalidates step where time is "rest" but a note is present', () => {
    const invalidPattern = {
      ...
      steps: [{ note: Note.C, time: Time.rest }],
    };
  ...
  it('invalidates step where note is set but time is not', () => {
    const invalidPattern = {
      ...
      steps: [{ note: Note.C, time: undefined }],
    };
    ...
    if (!result.success) {
      expect(result.error.issues[0].message).toBe(
        'Step 1: time must be set when a note is present.',
      );
    }
  });
