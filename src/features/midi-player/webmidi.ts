...
import {
  patternToMidiIter,
  patternToMidiNoteOffs,
} from '@/features/midi-player/pattern-to-midi-fsm';
import {
  MIDI_MESSAGE_PPQN,
  MidiMessage,
  parseSteps,
...
export const playPatternOverMidi = (
  ...
  const play = async () => {
    ...
    for (const { state, messages } of patternToMidiIter(steps)) {
      for (const message of messages) send(message);
      const cancelled = await delay(stepDuration);
      if (cancelled) {
        for (const message of patternToMidiNoteOffs(state)) send(message);
        return;
      }
    }
  };
